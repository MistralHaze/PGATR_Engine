...
static void SetupVulkan(const char** extensions, uint32_t extensions_count)
{
  ...
  // Create Vulkan Instance
  {
    ...
    // Enabling multiple validation layers grouped as LunarG standard validation
        ...
        // Setup the debug report _callback
...
static void SetupVulkanWindow(ImGui_ImplVulkanH_Window* wd, VkSurfaceKHR surface, int width, int height)
{
  ...
  if (res != VK_TRUE)
  {
    fprintf(stderr, "Error no WSI support on physical _device 0\n");
    exit(-1);
  }
...
#ifdef IMGUI_VULKAN_DEBUG_REPORT
  // Remove the debug report _callback
...
int main(int, char**)
{
  ...
  while (!glfwWindowShouldClose(window))
  {
    ...
    // 2. Show a simple _window that we create ourselves. We use a Begin/End pair to created a named _window.
    {
      ...
      ImGui::Checkbox("Demo Window", &show_demo_window);      // Edit bools storing our _window open/close state
      ImGui::Checkbox("Another Window", &show_another_window);

      ImGui::SliderFloat("float", &f, 0.0f, 1.0f);            // Edit 1 float using a slider from 0.0f to 1.0f
      ImGui::ColorEdit3("clear _color", (float*)&clear_color); // Edit 3 floats representing a _color
    ...
    {
      ImGui::Begin("Another Window", &show_another_window);   // Pass a pointer to our bool variable (the _window will have a closing button that will clear the bool when clicked)
